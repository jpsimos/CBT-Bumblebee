...
byte CANBus::getNextTxBuffer(byte *sendBuffer_out){
    byte stat;
    stat = this->readStatus();
	
	if( (stat & 0x4) != 0x4 ){
		*sendBuffer_out = SEND_TX_BUF_0;
		return 0;
	}

    if( (stat & 0x4) != 0x4 )   return 0;
    if( (stat & 0x10) != 0x10 ){
		*sendBuffer_out = SEND_TX_BUF_1;
		return 1;
	}
    if( (stat & 0x40) != 0x40 ){
		*sendBuffer_out = SEND_TX_BUF_0;
...
//Modified by jacob psimos 8/12/2016 jpsimos@gmail.com
...
//Added by Jacob Psimos to save memory <jpsimos@gmail.com>
void CANBus::send_X(byte SEND_TX_BUF_ID){
	digitalWrite(_ss, LOW);
...
//Added by Jacob Psimos to save memory <jpsimos@gmail.com>
void CANBus::readDATA_ff_X(byte readBuffer, byte* length_out,byte *data_out,
...
void CANBus::load_ff_X(byte bufferId, byte length,IDENTIFIER_INT identifier,byte *data,byte ide)
{
	...
	switch(bufferId){
		case 0:
			TX_BUFFER_ID_X = LOAD_TX_BUF_0_ID;
		break;
		case 1:
			TX_BUFFER_ID_X = LOAD_TX_BUF_1_ID;
		break;
		case 2:
			TX_BUFFER_ID_X = LOAD_TX_BUF_2_ID;
		break;
		default:
			TX_BUFFER_ID_X = 0xFF;
		break;
