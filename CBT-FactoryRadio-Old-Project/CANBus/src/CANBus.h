...
public:
    ...
    byte getNextTxBuffer(byte *sendBuffer_out);
	...
	void send_X(byte SEND_TX_BUF_ID);	//added by Jacob Psimos <jpsimos@gmail.com>
	...
	void readDATA_ff_X(byte readBuffer, byte* length_out,byte *data_out,IDENTIFIER_INT *id_out,byte *ide);
	...
	//Added load_ff_X to save memory - jacob psimos <jpsimos@gmail.com>
	void load_ff_X(byte bufferId, byte length,IDENTIFIER_INT identifier,byte *data,byte ide);
