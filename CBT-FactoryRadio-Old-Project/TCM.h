...
#define TCM_STATUS_ENABLED 0x01
#define TCM_STATUS_ENGAUGED 0x08
#define TCM_STATUS_STARTED 0x20
...
void TCM::tcm_handle_doublePress(){
  ...
  if(lastButtonPress + 500 <= now){
    ...
    if( !(status & TCM_STATUS_ENABLED) ){
      status &= ~TCM_STATUS_ENABLED;
    }else{
      status |= TCM_STATUS_ENABLED;
    }
    lastButtonPress = now;
...
void TCM::Shift(const byte dir){
  ...
  if(dir == TCM_SHIFT_NORMAL){
    ...
    switch(currentGear){
      ...
      break;
      case TCM_GEAR_3:
        if(dir == TCM_SHIFT_DOWN && currentSpeed < 51){
          shift = 0xA0;
...
void TCM::sendMessage(byte *data, const byte buflen, const byte msglen){
  ...
  memcpy(msg.frame_data, data, buflen);
