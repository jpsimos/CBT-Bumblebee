...
#define RADIO_DISP_SET_DELAY 150
...
#define RADIO_DISP_MODE_GPS_1 9
#define RADIO_DISP_MAX 9              // Keep updated to maximum numeric RADIO_DISP_MODE value
...
Radio::Radio(){
  //radioQueue = QueueArray<Message>(4);
  rateLimit = false;
...
void Radio::tick(){
  ...
  if(satUpdate){
     ...
     if(rateLimit && satUpdateIterations > 0){
      SetDisplayText("\x0A\x0A\x0A\x0A\x0A\x0A", 6, RADIO_DISP_XM_CAT);
      satUpdateIterations--;
     }

     if(satUpdateIterations == 0){
      this->satUpdate = false;
  ...
  if(!radioQueue.isEmpty() && dispatchRateLimit){
    Message msg = radioQueue.pop();
    writeQueue.push(msg);
    lastRadioTx = now;
  }else if(radioQueue.isEmpty() && dispatchRateLimit){
    Update();
  }
}
...
void Radio::Update(){
  ...
  switch(this->mode){
    ...
    case RADIO_DISP_MODE_THROTTLE:
      ...
      if(brakes){
        consumed = snprintf((char*)radioBuffer, 19, "Throttle: %s", "brakes");
      }else{
        memcpy((char*)radioBuffer, "Throttle: ", 10);
        if(throttle >= 0xFE){
          radioBuffer[10] = '1';
          radioBuffer[11] = '0';
          radioBuffer[12] = '0';
    ...
    case RADIO_DISP_MODE_FUEL_REMAIN:
      memcpy((char*)radioBuffer, "Fuel = ", 7);
      dtostrf(fuelF, 4, 1, (char*)(radioBuffer + 7));
      memcpy((char*)(radioBuffer + 11), "  gal", 5);
      consumed = 16;
    break;
    ...
    case RADIO_DISP_MODE_INTAKE:
      memcpy((char*)radioBuffer, "Intake: ", 8);
    ...
    case RADIO_DISP_MODE_GPS_1:
      consumed = snprintf((char*)radioBuffer, 19, "%3u ft %3u deg", elevation, heading);
    break;
    default:
      memcpy((char*)radioBuffer, "ERROR", 5);
      consumed = 5;
    break;
  }
