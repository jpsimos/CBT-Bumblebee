...
void OBD::tick(){
  ...
  switch(ptrRadio->mode){
    case RADIO_DISP_MODE_MPG:
      this->interval = OBD_REQ_INTERVAL_MEDIUM;
      if(engineRunning){
        request(OBD_PID_MAF);
      }else{
        MPG = 0.0;
    ...
    case RADIO_DISP_MODE_BAROMETER:
      this->interval = OBD_REQ_INTERVAL_SLOW;
      request(OBD_PID_BAROMETER);
    break;
...
void OBD::process(const Message &msg){
  if(msg.frame_id == SENSOR_OBD_RESP){
    switch(msg.frame_data[3]){
      ...
      break;
      case OBD_PID_BAROMETER:
        barometricPressure = msg.frame_data[4];
        ptrRadio->update = true;
      break;
      ...
      case OBD_PID_OIL_TEMP:
        oilTemp = ((((float)msg.frame_data[4]) - 40.0) * 1.8) + 32.0;
        ptrRadio->update = true;
      break;
    }
  }
